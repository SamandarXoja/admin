...
function Three() {
  const { parentId, secondId, threeId } = useParams();
  ...
  async function fecthmaterialData() {
    try {
      ...
      const response = await axios.get(
        `http://147.45.107.174:5000/api/material/materialCgId/${threeId}`,
        {
          headers: {
  ...
  async function handleEdit() {
    try {
      ...
      const data = {
        materialCategoryId: threeId, // id выбранной категории
        number,
        name,
        measurement,
        price: Number(price),
