...
function Second() {
  const { parentId, secondId } = useParams();
  ...
  async function fetchmaterialData() {
    try {
      ...
      const response = await axios.get(
        `http://147.45.107.174:5000/api/material/materialCgId/${secondId}`,
        {
          headers: {
  ...
  async function handleEdit() {
    try {
      ...
      const data = {
        materialCategoryId: secondId, // id выбранной категории
        number,
        name,
        measurement,
        price: Number(price),
